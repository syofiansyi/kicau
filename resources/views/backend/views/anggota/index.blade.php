    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table id="kt_datatable_zero_configuration" class="table table-row-bordered gy-5">
                ...
                <tbody>
                    @foreach ($anggota as $hap)
                        <tr data-item-id="1">
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $hap->nama_pemilik }}</td>
                            <td>{{ $hap->nama_burung }}</td>
                            <td>{{ $hap->alamat }}</td>
                            <td><img class="tbl-thumb" src="{{ asset('Upload/anggota/' . $hap->photo) }}" alt="No img"
                                    style="height:40px; width:40px " />
                            </td>
                            <td class="actions">
                                <a href="{{ route('backend.anggota.edit', $hap->id) }}" class="on-default mx-1"><i
                                        class="fas fa-pencil-alt"></i></a>
                                <form action="{{ route('backend.anggota.destroy', $hap->id) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body py-lg-10 px-lg-10">
                ...
                <div class="stepper stepper-pills stepper-column d-flex flex-column flex-xl-row flex-row-fluid"
                    ...
                    <form class="row g-3" action="{{ route('backend.anggota.store') }}" enctype="multipart/form-data"
                        method="POST">
                        @csrf
