            ...
            <tbody>
                @foreach ($news as $hap)
                    <tr data-item-id="1">
                        ...
                        <td>{{ $hap->title }}</td>
                        <td><img class="tbl-thumb" src="{{ asset('Upload/news/' . $hap->photo) }}" alt="No img"
                                 style="height:40px; width:40px " />
                        </td>
                        <td>{{ $hap->category == 0 ? 'News' : 'Hot' }}</td>
                        ...
                        <td class="actions">
                            <a href="{{ route('edit.news', $hap->id) }}" class="on-default mx-1"><i
                                    class="fas fa-pencil-alt"></i></a>
                            <a href="{{ route('delete.news', $hap->id) }}" class="on-default mx-1" id="delete"><i
                                    class="far fa-trash-alt"></i></a>
                                    @if ($hap->status == 1)
                                <a href="{{ route('news.inactive', $hap->id) }}" type="button" class="on-default mx-1">
                                    <i class="fa fa-eye" class="on-default"></i></i></a>
                            @else
                                <a href="{{ route('news.active', $hap->id) }}" type="button" class="mx-1"> <i
        ...
        <div class="modal-content">
            ...
            <div class="modal-body py-lg-10 px-lg-10">
                ...
                <div class="stepper stepper-pills stepper-column d-flex flex-column flex-xl-row flex-row-fluid" id="kt_modal_create_app_stepper">
                    <form class="row g-3" action="{{ route('store.news') }}" enctype="multipart/form-data" method="POST">
                        @csrf


                        <div class="form-group col-6">
                            <label for="inputEmail4">Judul</label>
                            <input type="text" class="form-control" id="inputEmail4" placeholder="Judul Berita" name="title">
                        </div>
                        <div class="form-group col-6">
